// Shared Kenyan-localization pools for the Grade 6 Numbers strand.
// Kept here (not inline per-file) so the same wide, real name/place variety threads through every
// skill's scenarios without 16 separate copies of the same arrays — same principle as
// preTechnicalG7/metallicMaterials.ts's place(rng)/name(rng), just centralized since this strand
// has 16 files sharing the same Kenyan-context flavor.
...
// "Count of X" real-world subjects for whole-number ordering/rounding/place-value scenarios.
// Each entry may contain a {place} token, substituted by the caller with place(rng).
export const COUNT_SCENARIO_SUBJECTS = [
  "learners enrolled in Grade 6 across primary schools in {place}",
...
// Item + typical unit-price (KES) contexts for multiplication (up to 4-digit x 2-digit).
...
// Fraction real-life "used {a}... then {b}..." scenario contexts (item + two activities).
...
// Measurement/decimal real-world contexts (subject + unit) for decimal addition/subtraction/percentage work.
...
// Real-life-rule contexts for forming inequalities (subject + unit + a plausible boundary range).
export const RULE_CONTEXTS = [
  { subject: "age (in years) to join the school football team", unit: "years", boundaryMin: 9, boundaryMax: 15 },
  { subject: "weight (in kg) of a parcel a courier in {place} will accept", unit: "kg", boundaryMin: 5, boundaryMax: 40 },
  { subject: "number of learners allowed on a school bus from {place}", unit: "learners", boundaryMin: 20, boundaryMax: 60 },
  { subject: "speed (in km/h) allowed on a road near a school in {place}", unit: "km/h", boundaryMin: 20, boundaryMax: 60 },
  { subject: "height (in cm) to ride the swing at a {place} playground", unit: "cm", boundaryMin: 90, boundaryMax: 140 },
  { subject: "amount of pocket money (in KES) a parent in {place} allows per week", unit: "KES", boundaryMin: 50, boundaryMax: 500 },
  { subject: "mass (in kg) of luggage allowed on a bus route through {place}", unit: "kg", boundaryMin: 15, boundaryMax: 30 },
  { subject: "number of goals a team from {place} must score to qualify", unit: "goals", boundaryMin: 2, boundaryMax: 10 },
  { subject: "temperature (in °C) needed for seedlings to germinate on a farm near {place}", unit: "°C", boundaryMin: 15, boundaryMax: 30 },
  { subject: "number of members needed to register a self-help group in {place}", unit: "members", boundaryMin: 5, boundaryMax: 20 },
  { subject: "marks needed on a test to pass at a school in {place}", unit: "marks", boundaryMin: 30, boundaryMax: 70 },
  { subject: "capacity (in people) of a matatu operating from {place}", unit: "people", boundaryMin: 10, boundaryMax: 33 },
  { subject: "distance (in km) a delivery rider from {place} covers for a flat fee", unit: "km", boundaryMin: 2, boundaryMax: 15 },
  { subject: "number of trees a household in {place} must plant under a county rule", unit: "trees", boundaryMin: 2, boundaryMax: 10 },
  { subject: "volume (in litres) a water tank in {place} must hold to serve a family", unit: "litres", boundaryMin: 200, boundaryMax: 2000 },
  { subject: "number of years of school attendance required before a certificate in {place}", unit: "years", boundaryMin: 4, boundaryMax: 8 },
  { subject: "mass (in kg) of a bag a Grade 6 learner in {place} carries to school", unit: "kg", boundaryMin: 2, boundaryMax: 8 },
  { subject: "amount (in KES) a stall owner in {place} must pay for a market licence", unit: "KES", boundaryMin: 100, boundaryMax: 1000 },
  { subject: "number of chickens allowed in a backyard coop in {place}", unit: "chickens", boundaryMin: 5, boundaryMax: 50 },
  { subject: "time (in minutes) allowed for a break at a school in {place}", unit: "minutes", boundaryMin: 10, boundaryMax: 40 },
  { subject: "number of passengers a boda boda in {place} is allowed to carry", unit: "passengers", boundaryMin: 1, boundaryMax: 2 },
  { subject: "mass (in kg) of maize a farmer near {place} needs to qualify for a cooperative bonus", unit: "kg", boundaryMin: 100, boundaryMax: 1000 },
  { subject: "number of runs a batting team from {place} needs to win a match", unit: "runs", boundaryMin: 50, boundaryMax: 200 },
  { subject: "age (in years) to obtain a boda boda licence near {place}", unit: "years", boundaryMin: 16, boundaryMax: 20 },
  { subject: "amount of water (in litres) rationed per household in {place} during a drought", unit: "litres", boundaryMin: 20, boundaryMax: 100 },
  { subject: "number of pupils needed to open a new stream at a school in {place}", unit: "pupils", boundaryMin: 20, boundaryMax: 45 },
  { subject: "distance (in metres) spectators must stay from a football pitch in {place}", unit: "metres", boundaryMin: 2, boundaryMax: 10 },
  { subject: "capacity (in kg) of maize a single sack sold in {place} can hold", unit: "kg", boundaryMin: 50, boundaryMax: 90 },
  { subject: "number of years a water pump near {place} is guaranteed to run", unit: "years", boundaryMin: 2, boundaryMax: 10 },
  { subject: "height (in m) of a fence required around a farm pond near {place}", unit: "m", boundaryMin: 1, boundaryMax: 3 },
  { subject: "number of members required to form a savings group (chama) in {place}", unit: "members", boundaryMin: 5, boundaryMax: 30 },
] as const;
