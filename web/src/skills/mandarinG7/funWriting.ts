...
const FILL_ITEMS: { before: string; after: string; answer: string; gloss: string }[] = [
  { before: "Wǒ ", after: " tīng yīnyuè.", answer: "zhōumò", gloss: "我周末听音乐。(I listen to music on weekends.)" },
  { before: "Fàngjià de shíhòu, wǒ hé péngyǒu ", after: "。", answer: "liáotiān", gloss: "放假的时候，我和朋友聊天。(During the holidays, I chat with friends.)" },
];

const ORDER_SETS: { chunks: string[]; sentence: string; gloss: string }[] = [
  { chunks: ["周末", "我喜欢看电影", "和打球。"], sentence: "周末我喜欢看电影和打球。", gloss: "On weekends I like watching movies and playing ball." },
];
...
export const funWriting: Skill = {
  ...
  description: "Guided writing — vocabulary and word order for leisure activities and how often you do them.",
  generate(rng) {
    ...
    if (branch === "categorize") {
      const frequency = shuffle(rng, VOCAB.filter((v) => v.tag === "frequency")).slice(0, 3);
      const activity = shuffle(rng, VOCAB.filter((v) => v.tag === "activity")).slice(0, 3);
      const chosen = shuffle(rng, [...frequency, ...activity]);
      const correctBucket: Record<string, string> = {};
      for (const v of frequency) correctBucket[v.hanzi] = "frequency";
      for (const v of activity) correctBucket[v.hanzi] = "activity";
      ...
      return {
        ...
        explanation: [...frequency, ...activity].map((v) => `"${v.hanzi}" is a ${correctBucket[v.hanzi] === "frequency" ? "time/frequency word" : "leisure activity"}.`).join(" "),
      };
    }
    ...
    if (branch === "mc") {
      ...
      return {
        ...
        layout: "list",
        hint: "Match the meaning to the exact word, not just a similar leisure-related one.",
