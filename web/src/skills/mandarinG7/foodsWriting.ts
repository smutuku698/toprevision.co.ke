...
const FILL_ITEMS: { before: string; after: string; answer: string; gloss: string }[] = [
  { before: "Māma, wǒmen qù ", after: " mǎi dōngxi ba.", answer: "chāoshì", gloss: "妈妈，我们去超市买东西吧。(Mom, let's go to the supermarket to buy things.)" },
  { before: "Mǎi liǎng jīn niúròu hé yì jīn ", after: "。", answer: "jīròu", gloss: "买两斤牛肉和一斤鸡肉。(Buy two jin of beef and one jin of chicken.)" },
];

const ORDER_SETS: { chunks: string[]; sentence: string; gloss: string }[] = [
  { chunks: ["我们去超市", "买牛肉", "和面包。"], sentence: "我们去超市买牛肉和面包。", gloss: "We go to the supermarket to buy beef and bread." },
];
...
export const foodsWriting: Skill = {
  ...
  description: "Guided writing — punctuation, spelling, and paragraph writing for food shops and quantities.",
  generate(rng) {
    ...
    if (branch === "match") {
      ...
      return {
        ...
        targets,
        correctMap,
        hint: "Some words name a shop; others name a food item.",
    ...
    if (branch === "categorize") {
      ...
      const foods = shuffle(rng, VOCAB.filter((v) => v.tag === "food")).slice(0, 3);
      const chosen = shuffle(rng, [...shops, ...foods]);
      ...
      return {
        ...
        correctBucket,
        hint: "A shop is a place; a food item is something you'd buy there.",
        explanation: [...shops, ...foods].map((v) => `"${v.hanzi}" is a ${correctBucket[v.hanzi] === "shop" ? "shop" : "food item"}.`).join(" "),
      };
    }
    ...
    if (branch === "mc") {
      ...
      return {
        ...
        layout: "list",
        hint: "Match the meaning to the exact word, not just a similar food-related one.",
